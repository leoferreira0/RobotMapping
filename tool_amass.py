...
def amass_(client, domain):
    scanner = 'Amass'
    
    os.system("rm /tool/robot_prefect/tmp/amass.tmp")
    os.system("touch /tool/robot_prefect/tmp/amass.tmp")
    
    os.system("docker run --rm -v "+"/tool/robot_prefect/tmp:/tmp "+"tool-kali:1.0 amass enum -o /tmp/amass.tmp -d " + domain)

    with open ('/tool/robot_prefect/tmp/amass.tmp') as file:
